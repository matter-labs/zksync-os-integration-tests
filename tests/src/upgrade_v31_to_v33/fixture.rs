...
/// Upgrade-env input for local anvil fixtures, committed in era-contracts.
/// Same content as `local.toml`, but its permanent-values twin (paired by
/// basename) has no `[legacy_gateway]` section — this fixture never had a
/// gateway chain, so the stage-2 decommission calls generated from that
/// section would revert with `SettlementLayersMustSettleOnL1()`. (Worse:
/// `permanent-values/local.toml` names chain 506 as the legacy gateway, which
/// is this fixture's only chain.)
///
