...
pub struct ExecuteManifestArgs {
...
pub async fn run(args: ExecuteManifestArgs) -> Result<()> {
    let fund = args
        .fund_targets
...
/// Apply bundles at positions `[start_index..end_index)` in the manifest.
/// `end_index` defaults to the end of the manifest when `None`.
/// Used by `apply --broadcast` to skip bundles from earlier commands
/// (e.g. ecosystem bundles from `bootstrap`) and to apply a specific slice
/// when resuming a dry-run workflow.
pub async fn apply_manifest_from(
    ...
    let cap = end_index
        .unwrap_or(all_bundles.len())
        .min(all_bundles.len());
    let new_bundles = &all_bundles[start_index.min(cap)..cap];
...
pub fn build_key_map(
    ...
    for key in &all_keys {
        let signer: PrivateKeySigner = key
            .parse()
            .map_err(|_| anyhow::anyhow!("invalid private key (key redacted)"))?;
