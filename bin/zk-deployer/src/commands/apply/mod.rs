...
/// The pubdata content a chain is initialized with. A validium is logs-only by
...
/// LOGS_ONLY pubdata content the chain is initialized with, not this registration:
/// a logs-only validium registers exactly like a rollup (blobs DA validator,
/// `PubdataPricingMode::Rollup`), because it does publish its logs region to L1.
fn resolve_da(
    chain: &ChainIntent,
    vm_type: VMOption,
