...
const INPUTS = ["package.json", "package-lock.json", "render_plonk_verifier.js"];
...
function prepare(root) {
  ...
  const key = path.join(root, "tools/verifier-gen/data/ZiSK_plonk_verification_key.json");
  const recipe = hash(
    Buffer.concat([
  ...
  try {
    ...
    fs.copyFileSync(key, path.join(staging, "verification_key.json"));
    ...
    process.stderr.write(
      run(process.execPath, ["render_plonk_verifier.js", "verification_key.json", "src/PlonkVerifier.sol"], {
        cwd: staging,
      })
    );
...
function main() {
  const [command, contractsRoot, ...rest] = process.argv.slice(2);
  const args = rest[0] === "--" ? rest.slice(1) : rest;
  if (!contractsRoot || !["prepare", "deploy", "test"].includes(command) || (command === "prepare" && args.length)) {
    throw new Error("Usage: node zisk-backend.js prepare|deploy|test <contracts-root> [-- arguments]");
  }
  ...
  const root = path.resolve(contractsRoot);
  const artifact = prepare(root);
  if (command === "prepare") {
